...
class SaveBox(Debris):
    ...
    def SaveAction(self, player):
        if self.cb is not None and self.triggered == False:
...
class FloatingFireExtinguisher(DynamicBox):
    ...
    def __init__(self, parent, fe, power, create_data=None):
        ...
        if fe is None:
            ...
            pos = (
                Point(*self.parent.body.GetWorldPoint(fe.base_pos.to_vec()))
                / self.parent.physics.scale_factor
            )
        ...
        self.middle = self.bl + self.half_size
        super(FloatingFireExtinguisher, self).__init__(
            self.parent.physics, self.bl, self.tr, self.texture_coords
        )
...
class FireExtinguisher(object):
    ...
    def SetPositions(self):
        ...
        self.base_pos = Point(
            *self.parent.body.GetLocalPoint((centre[0] + vector.real, centre[1] + vector.imag))
        )
    ...
    def PhysUpdate(self):
        ...
        if self.squirting:
            ...
            self.parent.body.ApplyForce(
                (vector.real, vector.imag), self.parent.body.GetWorldPoint(self.middle.to_vec())
            )
...
class Player(DynamicBox):
    ...
    def __init__(self, physics, bl, angle=0):
        ...
        Player.filter_id -= 1
        self.resting_hand_positions = (self.midpoint * Point(0.8, 1), self.midpoint * Point(-0.8, 1))
        self.current_hand_positions = self.resting_hand_positions
        self.shoulders = (self.midpoint * Point(0.8, 0), self.midpoint * Point(-0.8, 0))
        self.arms = [
            PlayerArm(self, (self, self.shoulders[0]), (self, self.resting_hand_positions[0])),
    ...
    def PhysUpdate(self):
        ...
        if self.push_start is not None:
    ...
    def Grab(self, obj, pos):
        ...
        if self.joints:
            self.Ungrab()
        if isinstance(obj, FireExtinguisher):
            obj = obj.parent
        phys_pos = pos * self.physics.scale_factor
